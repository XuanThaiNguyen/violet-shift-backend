...
describe("breakWorkLogIntoSegments", () => {
    ...
    describe("Multi-day scenarios", () => {
        ...
        it("should break work log across three days", () => {
            ...
            const uniqueDays = new Set(
                segments.map(s => new Date(s.from).toDateString())
            );
        ...
        it("should handle multiple holidays in multi-day period", () => {
            ...
            const segments = breakWorkLogIntoSegments(startTime, endTime, timeRules, [holiday1, holiday2]);
    ...
    describe("Edge cases", () => {
        it("should handle work log spanning midnight", () => {
            ...
            const uniqueDays = new Set(
                segments.map(s => new TZDate(s.from, timezone).toDateString())
            );
    ...
    describe("Timezone-sensitive scenarios", () => {
        ...
        it("should correctly handle timezone differences - same UTC time, different local times", () => {
            ...
            const SydneySegmentDays = new Set(sydneySegments.map(s => new TZDate(s.from, sydneyTimezone).toDateString()));
            const PerthSegmentDays = new Set(perthSegments.map(s => new TZDate(s.from, perthTimezone).toDateString()));
